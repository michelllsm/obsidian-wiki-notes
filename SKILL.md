...
| 指令 | 别名 | 什么时候用 | 输出目录 |
|------|------|-----------|---------|
| `-day` | `-daily`, `-每日笔记` | 一天结束时，记录今天学到的、遇到的问题、明天要做的。AI 自动读取当前对话上下文 + 全局记忆 + 今日已有笔记 | `📅每日笔记/` |
| `-note` | `-topic`, `-主题` | 收录一篇文章/视频/播客，AI 帮你提炼摘要和要点 | `📚主题笔记/` |
| `-input` | `-灵感`, `-输入` | 脑子里闪过一个想法，快速记下来，不需要完整结构 | `📚主题笔记/` |
| `-week` | `-weekly`, `-每周笔记` | 周末复盘。AI 自动读取本周所有每日笔记 + 当前对话汇总 | `📆每周笔记/` |

### 📤 输出与跟踪

| 指令 | 别名 | 什么时候用 | 输出目录 |
|------|------|-----------|---------|
| `-output` | `-out`, `-创作`, `-输出` | 你的原创内容：文章、帖子、分享稿（不与收录混放） | `📝创作输出/` |
| `-task` | `-todo`, `-待办`, `-跟踪` | 需要持续跟踪的任务：有背景、有完成标准、有进度 | `📋待跟踪任务/` |
...
| 指令 | 别名 | 什么时候用 | 输出目录 |
|------|------|-----------|---------|
| `-ingest` | `-归类`, `-classify`, `-整理` | 收件箱堆了一堆裁剪，让 AI 逐篇判断分类并归档 | 从 `📎Clippings/` 到各分类 |
| `-compile` | `-整合`, `-编译` | 某个主题攒了好几篇笔记，合并成一篇结构化总览 | `📚主题笔记/综合/` |
| `-check` | `-检查`, `-健康检查` | 定期体检：找出孤立笔记、过期内容、缺失链接 | 输出到对话（不生成文件） |
| `-update` | `-iterate`, `-迭代` | 想到了改进笔记工作流本身的点子 | `💡笔记工作流优化idea/` |
...
### 1.5.6 `-ingest` 收件箱整理指令

**触发**：用户说 `-ingest`、`-归类` 或 `-classify`
...
**`-week` 执行时，AI 自动读取**：
1. 本周所有 `📅每日笔记/` 文件（按日期范围扫描）
2. 当前对话上下文
3. 全局记忆

> 注意：AI 无法跨对话窗口读取其他对话的聊天记录。如果本周有重要内容在其他对话中，用户需要简要补充。
